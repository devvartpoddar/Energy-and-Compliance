...
would argue that if a states policies are in line with federal rulings,
...
Energy policy in the United States is formulated at both the federal and
the state level. Federal level policies tend to be more financially
based, and rely primarily on Tax Credits and Cost Recovery Systems. In
contrast, state level policies can take a wide variety of forms
including feed-in-tariffs, and renewable energy requirements
\citep{FERC2016}. Hence, it is at the state level that there is
significant divergence in policy as all states are subject to the
overarching policies made at the federal level (and, in tandem, all
rulings that are made by FERC).
...
\caption{Renewable Energy Policies}\tabularnewline
\toprule
...
requirements set out by other policies \citep{Wang2016}.

\section{Methodology}\label{methodology}

\subsection{Overview}\label{overview}

We use a most different systems design (MDSD) to evaluate changes in
prices at the state level. A MDSD evaluates when and why different
systems have the same outcome \citep{Levy2008}. For our analysis, the
dependent variables are electricity pricing and the implementation of
RPS policies at the state level. We wonder if changes in judicial
rhetoric at the federal level impacts these two variables in the same
way in different contexts. Theoretically, if increased prices are linked
to a greater emphasis on renewable policies, then we should see
increased electricity prices and increased implementation of RPS
policies in the different states if the judicial rhetoric changes.

\subsection{Operationalizing
Reverberation}\label{operationalizing-reverberation}

This paper relies on the assumption that federal energy court decisions
reverberate in state energy policy. We define energy policy
reverberation as the impact of three intertwined variables. The first is
frequency. We argue that the more frequently FERC uses particular terms,
the more likely the court decisions are to reverberate in the state
context. This is because the more a federal court makes statements about
a topic, the more likely this topic will arguably stay in the mind of a
state politician or state advocates. Repetition, here, is key for
getting into the long-term memory of particular individuals.
...
choice of some terms is more likely to encourage a positive association
...
carry positive connotations with regards to renewable energy.

The third variable is that of action terminology. Here, words that
indicate that the court is ordering a particular corporation or person
to take action are correlated with situations where the individual does
not have a choice as to whether they will adhere to the ruling. The
action terms that will be searched for are order, revoke, and comply.

These three terms will be used to analyze whether changes in judicial
terminology impact electricity pricing and RPS implementation.

\subsection{Textual Analysis}\label{textual-analysis}

The judicial terminology was obtained from the FERC online
commissioners, with the PDF files being scraped from their website. The
frequency of the orders is highlighted in Chart 1 below. Notably, there
has been an increased proportion of decisions made by FERC since 2005,
with a dramatic peak in 2015. Not all of these decisions will relate to
renewable energy policy, but the increased use of the court to rule on
legal disputes indicates that it may be perceived to have greater
legitimacy in ruling on the cases. This increased legitimacy would
theoretically lead to a greater likelihood of its policies reverberating
in state contexts.

\includegraphics{PDF_files/figure-latex/copy2-1.pdf}

We cleaned the pdf text to get to a measure of legitamcy / compliance.
The first stage of the analysis involved the use of \textbf{Treetagger},
...
We controlled for several variables to ensure that we are not
overemphasizing the impact of judicial rhetoric on electricity prices
and RPS policy. Each of the relevant variables was obtained using web
scrapping (see Appendix 1). The variables were then matched based on
year, month, and state in question. For variables that did not change
over the course of a year, or between years, such as the proportion of
the popular vote that went to each party in the relevant elections, the
same numbers were applied to all observations until they changed. Hence,
the votes for political parties from 2000 were counted as the same for
all months in 2000, 2001, 2002, and 2003. Those from 2004 were counted
for all months in 2004, 2005, 2006, and 2007, and so on.

Specifically, we obtained data on a variety of variables that could
impact electricity pricing. For example, variables such as political
leanings, or whether the state votes democrat or republican, may be
correlated with different odds of implementing RPS policies. Further,
weather patterns, such as particularly hot summers, may make electricity
more expensive because of increased demand. Finally, electricity market
deregulation may impact electricity prices regardless of whether the
state has engaged in RPS policies. These variables are specified in the
following table:

\begin{longtable}[c]{@{}lllllllr@{}}
\caption{Variable Overview}\tabularnewline
\toprule
X & Prices & RPS & Dereg. & Order & Renewable & Context &
Temp.\tabularnewline
\midrule
\endfirsthead
\toprule
...
Median & 10.16 & 0 & 0 & 1.41 & 0.08 & 0.08 & 89.50\tabularnewline
Min & 4.95 & 0 & 0 & 0.11 & 0.01 & 0 & 2.00\tabularnewline
Max & 23.08 & 1 & 1 & 2.51 & 0.42 & 0.31 & 185.00\tabularnewline
Unit & USD & - & - & \% point & \% point & \% point & NA\tabularnewline
\bottomrule
\end{longtable}

There are a few things to take note of in this table. First, there is a
wide variance of electricity prices, ranging from 5 cents per
kilowatt-hour (kWh) to 23 cents per kWh. However, the majority of
pricing is at around the 10-11 cent mark. Second, there are fewer states
with RPS policies than without RPS policies (as indicated by the 0.43
mean), and fewer deregulated states (mean of 0.33). Finally, the order
variable has the most variance of the terminology variables (can be
anywhere from 2.51 percent of the text to 0.11 percent of the text). It
is also more likely to be used in judicial decisions. In contrast,
context and renewable have less variance and are used with less
frequency. This makes sense because courts are likely to use order terms
in cases that do not refer to renewable energy, whereas renewable
terminology would be unlikely to come up unless the case were to
specifically address renewable policy.

\subsection{Dependent Variable: Electricity
Pricing}\label{dependent-variable-electricity-pricing}

The key variable for the purposes of our analysis is that of electricity
prices. When the electricity prices are broken down by month, the trend
is for prices to be lowest in the winter, and highest in the summer.
This is important for our final model because it reveals that changes in
electricity pricing may differ as a result of month-by-month usage
changes, and not because of other factors. We will control for this in
our final regression.

\includegraphics{PDF_files/figure-latex/copy3-1.pdf}
...
Chart 3 presents the results of our analyses for the impact of order,
renewables and context terminology on electricity pricing. It also
includes the impact of RPS policies on electricity prices. It should be
noted that for each regression, we have controlled for the other noted
variables, as well as political affiliation, deregulation, and weather.
...
\includegraphics{PDF_files/figure-latex/copy5-1.pdf}

Curiously, the inclusion of the month fixed effects regression has a
substantial impact on the correlation between the terminology and RPS
variables, and electricity prices. Specifically, both context and
renewable variables drop to a negative effect. The frequency of these
variables is now correlated with a decrease in the price of electricity.
This means that as the appearance of both order and renewable words
within 30 words of each other increases by 1 percent, the correlated
cost of electricity goes down by 0.18 percent. If there is a 1 percent
increase in the number of times renewables are referenced (even without
being linked to an order term), this is correlated with a 0.15 percent
decrease in electricity prices. Our theoretical framework did not
predict these results, and these results suggest that there must be some
unspecified bridging variables between the implementation of renewable
energy policies and increased electricity costs. These bridging
variables could include companies buying into long-term contracts that
lock them in to more expensive renewable energy production. However,
this is subject for future research.

In contrast, order remains positively correlated with electricity prices
in both of the models (though with the month fixed effects model, the
coefficient is just above 0.00). RPS policies are also correlated with
increased electricity prices in both models. However, in our base fixed
effects model having an RPS policy is correlated with a 0.26 percent
increase in electricity prices, whereas in the month-to-month model it
is only correlated with a 0.04 percent increase in electricity prices.

\subsection{Two Step}\label{two-step}

We re-run our results using a two-stage least squares regression
displayed in the table below. RPS still has a staticially significant
and positive increase in renewable electricity prices. The effect
however remains constant, with RPS increasing prices by 0.04\%.

\begin{table}[!htbp] \centering 
  \caption{Two Stage Least Square Regressions} 
  \label{} 
...
R$^{2}$ & 0.786 \\ 
Adjusted R$^{2}$ & 0.780 \\ 
F Statistic & 2,446.652$^{***}$ (df = 14; 9336) \\ 
\hline 
\hline \\[-1.8ex] 
...
\section{Appendix: Web-Scrapped
Variables}\label{appendix-web-scrapped-variables}

\begin{longtable}[c]{@{}ll@{}}
\caption{Web Scrapped Variables}\tabularnewline
\toprule
Variable & Source\tabularnewline
\midrule
\endfirsthead
\toprule
Variable & Source\tabularnewline
\midrule
\endhead
Prices & EIA\tabularnewline
RPS & Wang (2016)\tabularnewline
Dereg & Electricchoice.com\tabularnewline
Electoral Votes & Electoral-Votes.com\tabularnewline
Weather & NOAA\tabularnewline
FERC Judicial Cases & FERC\tabularnewline
\bottomrule
\end{longtable}
