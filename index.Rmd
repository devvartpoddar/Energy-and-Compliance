...
reg.coef <- function() {
  ...
  coef.extract <- function(var.names, model, data = reg.data) {
    ...
    coef.table <- fe.reg[["coefficients"]] %>%
      ...
      mutate(variable = rownames(.),
             model = model) %>%
      filter(grepl("RPS|order.ave|context_30", variable)) %>%
  ...
  var.names <- paste(c("RPS", variable, "party", "temperature"), collapse = " + ")

  base <- coef.extract(var.names, "Base FE")

  # Running Fe with Dummies for months
  var.names <- paste(c("RPS", variable, "party", "temperature",
  ...
  # Running with yearly data instead of monthly
    var.names <- paste(c("RPS", variable, "Electric", "temperature",
                       "year"), collapse = " + ")

    year.fe <- coef.extract(var.names, "Year FE", year.data)

    final.table <- rbind(base, month.fe, median.fe, year.fe)

    return(final.table)
  }
  
  order.model <- model.fun()
  final.model <- model.fun("context_30") %>% rbind(order.model)
