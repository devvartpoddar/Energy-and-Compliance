...
data2 <- final.data %>%
  dplyr::summarise(x = 1.2 * mean(Prices)) %>%
  mutate(y = 420,
         label= "Mean Price")

ggplot(final.data) +
  geom_histogram(aes(Prices), bins = 25) +
  geom_linerange(aes(x = mean(Prices), ymin = 0, ymax = 440),
...
``` {r judicialdecisions, echo = FALSE, cache=3, warning = FALSE, message = FALSE}
...
```
``` {r wordcloud1, echo = FALSE, cache = 3, warning = FALSE}
par(mfrow=c(1,2))
